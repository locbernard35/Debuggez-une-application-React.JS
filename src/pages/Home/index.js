import Menu        from "../../containers/Menu"; // Correct   
import ServiceCard from "../../components/ServiceCard";
import EventCard   from "../../components/EventCard";
import PeopleCard  from "../../components/PeopleCard";

import "./style.scss";
import EventList   from "../../containers/Events";// Nos réalisations
import Slider      from "../../containers/Slider";// Correct
import Logo        from "../../components/Logo";
import Icon        from "../../components/Icon";
import Form        from "../../containers/Form";// Correct
...
const Page = () => {   
 ...
 const {data} = useData();
 // console.log(data?.events.length);//True => 18 length 
 // Tri par date décroissant
 const dataDesc = data?.events.sort((evtA, evtB) =>
 new Date(evtA.date) < new Date(evtB.date) ? 1 : -1
 );
 
 const last = dataDesc?.["0"]; // dernier date json
 // console.log(datadesc);
 // console.log(datadesc?.["0"].id);//=>desc

 // No-warning =>type validation
 const title = last?.title
 const titleString  = String(title)

 const cover = last?.cover
 const coverString  = String(cover)
